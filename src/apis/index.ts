...
import GetCustomerListResponseDto from "./dto/response/customer/get-customer-list.response.dto";
import { PostCustomerRequestDto, PatchCustomerRequestDto, PostCareRecordRequestDto } from "./dto/request/customer";
import GetCustomerResponseDto from "./dto/response/customer/get-customer.response.dto";
import { GetCareRecordListResponseDto } from "./dto/response/customer";
import PatchNurseRequestDto from "./dto/request/nurse/patch-nurse.request.dto";
...
const GET_NRSE_API_URL = (userId: string) => `${NURSE_MODUEL_URL}/${userId}`;
...
const POST_CARE_RECORD_LIST_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODULE_URL}/${customerNumber}/care-record`;
...
export const getNurseRequest = async (userId: string, accessToken: string) => {
  const responseBody = await axios.get(GET_NRSE_API_URL(userId), bearerAuthorization(accessToken))
...
export const postCareRecordRequest = async (requestBody: PostCareRecordRequestDto, customerNumber: number | string, accessToken: string) => {
  const responseBody = await axios.post(POST_CARE_RECORD_LIST_API_URL(customerNumber), requestBody, bearerAuthorization(accessToken))
    .then(responseDataHandler<ResponseDto>)
    .catch(responseErrorHandler);
  return responseBody;
};
...
export const postCareRequest = async (customerNumber: number | string, accessToken: string) => {
  const responseBody = await axios.get(GET_CUSTOMER_API_URL(customerNumber), bearerAuthorization(accessToken))
    .then(responseDataHandler<GetCustomerResponseDto>)
    .catch(responseErrorHandler);
  return responseBody;
...
// function: get care record 요청 함수 //
export const getCareRecordListRequest = async (customerNumber: number | string, accessToken: string) => {
  const responseBody = await axios.get(GET_CARE_RECORD_LIST_API_URL(customerNumber), bearerAuthorization(accessToken))
    .then(responseDataHandler<GetCareRecordListResponseDto>)
    .catch(responseErrorHandler);
  return responseBody;
};
...
const multipart = { headers: { 'Content-Type': 'multipart/fomr-data' } };
