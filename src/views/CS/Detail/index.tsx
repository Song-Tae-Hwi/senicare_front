...
import { GetCareRecordListResponseDto, GetCustomerResponseDto } from 'src/apis/dto/response/customer';
...
export default function CSDetail() {
  ...
  const changeDateFormant = (date: string) => {
  ...
  const getCareRecordListResponse = (responseBody: GetCareRecordListResponseDto | ResponseDto | null) => {
    const message =
      !responseBody ? '서버에 문제가 있습니다.' :
        responseBody.code === 'VF' ? '잘못된 접근입니다.' :
          responseBody.code === 'AF' ? '잘못된 접근입니다.' :
    ...
    const { careRecords } = responseBody as GetCareRecordListResponseDto;
  ...
  const onRecordContentChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
          ...
          <div className='table'>
            ...
            {viewList.map((careRecord, index) =>
              <div key={index} className='tr'>
                <div className='td-record-date'>{changeDateFormant(careRecord.recordDate)}</div>
          ...
          <div className='record-write-box'>
            <div className='record-write-content-box'>
              <div className='input-box' style={{ flex: 1 }}>
                ...
                <input className='input' value={recordContents} placeholder='내용을 입력하세요.' onChange={onRecordContentChangeHandler} />
              </div>
