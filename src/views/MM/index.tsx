...
function PatchBox({ unShow, toolNumber, getToolList }: PatchBoxProps) {
  ...
  const getToolResponse = (responseBody: GetToolResponseDto | ResponseDto | null) => {
    ...
    setName(name);
    setPurpose(purpose);
    setCount(String(count)); // number로 받아와 String으로 강제 형변환
  };
...
export default function MM() {
  ...
  const { currentPage, totalPage, totalCount, viewList, pageList, setTotalList, initViewList,
  ...
  const unShowPatchtBox = () => setShowPatchBox(false);
  ...
  return (
    <div id='mm-wrapper'>
      ...
      {showPatchBox && <PatchBox unShow={unShowPatchtBox} getToolList={getToolList} toolNumber={patchToolNumber} />}
      <div className='top'>
