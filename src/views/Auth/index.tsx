...
function SignUp({ onPathChange }: AuthComponentProps) {
            ...
            !responseBody ? '서버에 문제가 있습니다.' :
                responseBody.code === 'VF' ? '올바른 데이터가 아닙니다.' :
                    responseBody.code === 'TAF' ? '인증번호가 일치하지 않습니다.' :
                        responseBody.code === 'DBE' ? '서버에 문제가 있습니다 DBError.' :
                ...
                responseBody.code === 'VF' ? '올바른 데이터가 아닙니다.' :
                    responseBody.code === 'DI' ? '이미 사용중인 아이디입니다.' :
                        responseBody.code === 'DT' ? '중복된 전화번호 입니다.' :
                            responseBody.code === 'TAF' ? '인증번호가 일치하지 않습니다.' :
                                responseBody.code === 'DBE' ? '서버에 문제가 있습니다 DBError.' :
                                    responseBody.code === 'SU' ? '회원가입에 성공했습니다. ' : '';
        const isSuccessed = responseBody !== null && responseBody.code === 'SU';
        if (!isSuccessed) {
            alert(message);
            return;
    ...
    const onAuthNumberCheckClickHandler = () => {
        ...
        const requestBody: TelAuthCheckRequestDto = {
            telNumber, authNumber // 상태에 똑같은 이름으로 존재하여 콜럼 찍을 필요 X
        }
...
function signInResponse(value: SignInResponseDto | ResponseDto | null): SignInResponseDto | ResponseDto | PromiseLike<SignInResponseDto | ResponseDto | null> | null {
